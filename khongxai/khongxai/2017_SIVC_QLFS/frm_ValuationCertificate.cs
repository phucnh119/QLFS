...
namespace _2017_SIVC_QLFS
{
    public partial class frm_ValuationCertificate : Form
    {
        ...
        public void LoadGridByKeyword()
        {
            ...
            bs.DataSource = dg_Show.DataSource;
            bs.Filter = "VC_Number like '%" + txtkeyword.Text + "%' or Customer like '%" + txtkeyword.Text + "%' or ContractNumber like '%" + txtkeyword.Text + "%' ";
            dg_Show.DataSource = bs;
        ...
        {
            export2Excel(dg_Show, @"", "Báo cáo tổng hợp");
            MessageBox.Show("Xuất thành công trong document");
        }
        ...
        {
            BindingSource mt = new BindingSource();

            mt.DataSource = dg_Show.DataSource;

            mt.Filter = string.Format("Date_VC >= #{0:yyyy/MM/dd}# And Date_VC <= #{1:yyyy/MM/dd}#", dateTimePicker1.Value, dateTimePicker2.Value);
            dg_Show.DataSource = mt;
        }
    }
}
